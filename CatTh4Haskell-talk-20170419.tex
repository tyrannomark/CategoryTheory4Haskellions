...
\begin{frame}[fragile]{Initial Objects}

  \emph{Initial Objects}

  An \emph{initial object} is an object $0$ (a.k.a.\ $I$) in $\Obj{C}$, such
  that there is only a single mapping from it to any other type $A$:

  \[
  \begin{tikzcd}
    \forall A\in \Obj{C}, \left| \Hom{C}{0}{A} \right| = 1.
  \end{tikzcd}
  \]
  ...
  \[
    \begin{tikzcd}[row sep=tiny]
      & A \\
      0 \arrow[near end]{ur}{\exists!} \arrow["\exists!" description]{r} \arrow[swap,near end]{dr}{\exists!} & B \\
       & C
    \end{tikzcd}
  \]
  ...
  \begin{lstlisting}[frame=single]
    Void -- the type corresponding to 0, containing no instances
    absurd :: Void -> t
...
\begin{frame}[fragile]{Terminal Objects}
...
\begin{frame}[fragile]{Examples}
  ...
  \begin{tabular}{r l l l l}\toprule
>>>>>>> 78ad0526b0ba1f73ab0a22c580381b3a60a6e261
    & $\Cat{Set}$ & $\Cat{Hask}$ & $\Cat{POrd}$ & $\Cat{Cat}$ \\\midrule
    \textbf{Objects} & sets & types & items & small cats \\
    \textbf{Morphisms} & functions & functions & $a \leq b$ & functors \\
    \textbf{Composition} & $f \circ g$ & \lstinline!f.g! & transitivity & $F \circ G$ \\
    \textbf{Identity} & $1_A$ & {\lstinline!id!} & $a = a$ & $1_{\Cat{C}}$ \\
    \textbf{Initial obj\rlap{.}} & $\emptyset$ & \lstinline!Void! & lwr bnd & $\Cat{0}$ \\
    \textbf{Terminal obj\rlap{.}} & $\{*\}$ & \lstinline!()! & uppr bnd & $\Cat{1}$ \\\bottomrule
  \end{tabular}
<<<<<<< HEAD
  \end{center}
...
\begin{frame}[fragile]
  ...
  \begin{center}
    ...
    \textbf{Identity} & $1_A$ & {\lstinline!id!} & $a = a$ & $1_{\Cat{C}}$ \\
    \textbf{Initial obj\rlap{.}} & $\emptyset$ & \lstinline!Void! & lwr bnd & $\Cat{0}$ \\
    \textbf{Terminal obj\rlap{.}} & $\{*\}$ & \lstinline!()! & uppr bnd & $\Cat{1}$ \\
